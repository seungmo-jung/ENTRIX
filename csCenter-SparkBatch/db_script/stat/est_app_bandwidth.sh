...
function getSQL() {
	...
	local SQL="INSERT INTO est_app_bandwidth_$targetname(stat_$targetname, app_id, avg_mbps, min_mbps, max_mbps, chg_date) SELECT '$statdate' AS stat_$targetname, app_id, AVG(avg_mbps), MIN(min_mbps), MAX(max_mbps), '$nowdate' FROM est_app_bandwidth_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY app_id;"
...
daySQL="INSERT INTO est_app_bandwidth_day(stat_day, app_id, avg_mbps, min_mbps, max_mbps, chg_date) SELECT '$statday', app_id, AVG(avg_mbps), MIN(min_mbps), MAX(max_mbps), '$nowdate' FROM est_app_bandwidth_hour WHERE stat_hour LIKE '$statday%' GROUP BY app_id;"
