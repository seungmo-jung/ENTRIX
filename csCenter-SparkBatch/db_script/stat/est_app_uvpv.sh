...
function getSQL() {
	...
	local SQL="INSERT INTO est_app_uvpv_$targetname(stat_$targetname, app_id, uv, pv, chg_date) SELECT '$statdate' AS stat_$targetname, app_id, SUM(uv), SUM(pv), '$nowdate' FROM est_app_uvpv_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY app_id;"
...
then
	...
	daySQL="INSERT INTO est_app_uvpv_day(stat_day, app_id, uv, pv, chg_date) SELECT '$statday', app_id, SUM(uv), SUM(pv), '$nowdate' FROM est_app_uvpv_hour WHERE stat_hour LIKE '$statday%' GROUP BY app_id;"
