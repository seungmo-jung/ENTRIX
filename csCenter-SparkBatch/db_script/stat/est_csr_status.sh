...
function getSQL() {
	...
	local SQL="INSERT INTO est_csr_status_$targetname(stat_$targetname, csr_id, req_cnt, success_cnt, fail_cnt, success_rate, chg_date) SELECT '$statdate' AS stat_$targetname, csr_id, SUM(req_cnt), SUM(success_cnt), SUM(fail_cnt), IFNULL((SUM(success_cnt) / SUM(req_cnt) * 100), '0'), '$nowdate' FROM est_csr_status_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY csr_id;"
...
daySQL="INSERT INTO est_csr_status_day(stat_day, csr_id, req_cnt, success_cnt, fail_cnt, success_rate, chg_date) SELECT '$statday', csr_id, SUM(req_cnt), SUM(success_cnt), SUM(fail_cnt), IFNULL((SUM(success_cnt) / SUM(req_cnt) * 100), '0'), '$nowdate' FROM est_csr_status_hour WHERE stat_hour LIKE '$statday%' GROUP BY csr_id;"
