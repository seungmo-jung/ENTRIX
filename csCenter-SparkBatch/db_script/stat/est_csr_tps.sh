...
function getSQL() {
	...
	local SQL="INSERT INTO est_csr_tps_$targetname(stat_$targetname, so_id, csr_id, avg_tps, min_tps, max_tps, chg_date) SELECT '$statdate' AS stat_$targetname, so_id, csr_id, AVG(avg_tps), MIN(min_tps), MAX(max_tps), '$nowdate' FROM est_csr_tps_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY so_id, csr_id;"
...
daySQL="INSERT INTO est_csr_tps_day(stat_day, so_id, csr_id, avg_tps, min_tps, max_tps, chg_date) SELECT '$statday', so_id, csr_id, AVG(avg_tps), MIN(min_tps), MAX(max_tps), '$nowdate' FROM est_csr_tps_hour WHERE stat_hour LIKE '$statday%' GROUP BY so_id, csr_id;"
