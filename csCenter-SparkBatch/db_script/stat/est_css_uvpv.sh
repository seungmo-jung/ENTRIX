...
function getSQL() {
	...
	local SQL="INSERT INTO est_css_uvpv_$targetname(stat_$targetname, app_id, so_id, css_ip, uv, pv, chg_date) SELECT '$statdate' AS stat_$targetname, app_id, so_id, css_ip, SUM(uv), SUM(pv), '$nowdate' FROM est_css_uvpv_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY app_id, so_id, css_ip;"
...
then
	...
	daySQL="INSERT INTO est_css_uvpv_day(stat_day, app_id, so_id, css_ip, uv, pv, chg_date) SELECT '$statday', app_id, so_id, css_ip, SUM(uv), SUM(pv), '$nowdate' FROM est_css_uvpv_hour WHERE stat_hour LIKE '$statday%' GROUP BY app_id, so_id, css_ip;"
