...
function getSQL() {
	...
	local SQL="INSERT INTO est_mem_$targetname(stat_$targetname, so_id, host_name, avg_used, min_used, max_used, chg_date) SELECT '$statdate' AS stat_$targetname, so_id, host_name, AVG(avg_used), MIN(min_used), MAX(max_used), '$nowdate' FROM est_mem_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY so_id, host_name;"
...
daySQL="INSERT INTO est_mem_day(stat_day, so_id, host_name, avg_used, min_used, max_used, chg_date) SELECT '$statday', so_id, host_name, AVG(avg_used), MIN(min_used), MAX(max_used), '$nowdate' FROM est_mem_hour WHERE stat_hour LIKE '$statday%' GROUP BY so_id, host_name;"
