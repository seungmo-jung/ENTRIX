...
function getSQL() {
	...
	local SQL="INSERT INTO est_network_$targetname(stat_$targetname, so_id, network_id, port_no, avg_rx_bps, min_rx_bps, max_rx_bps, avg_tx_bps, min_tx_bps, max_tx_bps, chg_date) SELECT '$statdate' AS stat_$targetname, so_id, network_id, port_no, AVG(avg_rx_bps), MIN(min_rx_bps), MAX(max_rx_bps), AVG(avg_tx_bps), MIN(min_tx_bps), MAX(max_tx_bps), '$nowdate' FROM est_network_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY so_id, network_id, port_no;"
...
daySQL="INSERT INTO est_network_day(stat_day, so_id, network_id, port_no, avg_rx_bps, min_rx_bps, max_rx_bps, avg_tx_bps, min_tx_bps, max_tx_bps, chg_date) SELECT '$statday', so_id, network_id, port_no, AVG(avg_rx_bps), MIN(min_rx_bps), MAX(max_rx_bps), AVG(avg_tx_bps), MIN(min_tx_bps), MAX(max_tx_bps), '$nowdate' FROM est_network_hour WHERE stat_hour LIKE '$statday%' GROUP BY so_id, network_id, port_no;"
