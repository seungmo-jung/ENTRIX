...
function getSQL() {
	...
	local SQL="INSERT INTO est_so_ccu_$targetname(stat_$targetname, app_id, so_id, avg_ccu, min_ccu, max_ccu, chg_date) SELECT '$statdate' AS stat_$targetname, app_id, so_id, AVG(avg_ccu), MIN(min_ccu), MAX(max_ccu), '$nowdate' FROM est_so_ccu_$srcname WHERE stat_$srcname LIKE '$statdate%' GROUP BY app_id, so_id;"
...
daySQL="INSERT INTO est_so_ccu_day(stat_day, app_id, so_id, avg_ccu, min_ccu, max_ccu, chg_date) SELECT '$statday', app_id, so_id, AVG(avg_ccu), MIN(min_ccu), MAX(max_ccu), '$nowdate' FROM est_so_ccu_hour WHERE stat_hour LIKE '$statday%' GROUP BY app_id, so_id;"
